...
export const Tuner: React.FC<TunerProps> = ({ theme = 'dark' }) => {
  ...
  // Рефы для сглаживания (EMA)
  const smoothedCentsRef = useRef<number>(0);
  const smoothingFactor = 0.15; // Коэффициент плавности (0.05 - 0.2)
  ...
  const startTuner = async () => {
    ...
    try {
      ...
      // Попытка возобновить контекст (важно для мобильных браузеров)
      ...
      // Если iOS всё еще блокирует звук
      if (ctx.state === 'suspended') {
        setIsSuspended(true);
      } else {
        updatePitch();
  ...
  const updatePitch = () => {
    ...
    if (freq !== -1) {
      ...
      // Плавное возвращение стрелки в центр при тишине
  ...
  const autoCorrelate = (buffer: Float32Array, sampleRate: number) => {
    ...
    if (rms < 0.015) return -1; // Более строгий порог шума
    ...
    <div className={`flex flex-col h-full pt-[env(safe-area-inset-top)] overflow-hidden transition-colors duration-300 ${isDark ? 'bg-black' : 'bg-[#f8f9fa]'}`}>
      ...
      {isRequesting ? (
        <div className="flex-1 flex flex-col items-center justify-center p-10 text-center">
          ...
          <p className={`font-black text-xs uppercase tracking-widest ${isDark ? 'text-zinc-600' : 'text-zinc-400'}`}>Connecting Mic...</p>
        </div>
      ) : error ? (
        <div className="flex-1 flex flex-col items-center justify-center p-10 text-center animate-in fade-in duration-500">
          ...
          <h2 className={`text-xl font-bold mb-2 ${isDark ? 'text-white' : 'text-zinc-900'}`}>No Mic Access</h2>
        ...
        <div className="flex-1 flex flex-col items-center justify-center p-10 text-center animate-in zoom-in-95 duration-500">
           ...
           <p className="text-zinc-500 text-[10px] font-bold uppercase tracking-widest mt-2">iOS Security Verification</p>
        </div>
      ) : (
        <div className="flex-1 flex flex-col items-center justify-between py-10 animate-in fade-in duration-700">
          ...
          <div className="flex flex-col items-center gap-4">
             <div className={`px-4 py-2 rounded-full border flex items-center gap-2 ${isDark ? 'bg-zinc-900/50 border-white/5' : 'bg-white border-zinc-200'}`}>
                ...
                <span className={`text-[8px] font-black uppercase tracking-widest ${isDark ? 'text-zinc-500' : 'text-zinc-400'}`}>Monitoring</span>
             </div>
          </div>
        </div>
      )}
