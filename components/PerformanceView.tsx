...
export const PerformanceView: React.FC<PerformanceViewProps> = ({ song, onClose, onEdit, onUpdateTranspose }) => {
  ...
  const handleScroll = useCallback(() => {
    if (scrolling && containerRef.current) {
      const speed = (scrollSpeed / 100) * 1.2;
      ...
      if (containerRef.current.scrollTop + containerRef.current.clientHeight >= containerRef.current.scrollHeight) {
        ...
        scrollIntervalRef.current = requestAnimationFrame(handleScroll);
      }
    }
  ...
  const renderContent = () => {
    ...
    return text.split('\n').map((line, i) => {
      // Регулярка для разделения, сохраняющая сами аккорды в массиве
  ...
  return (
    ...
    >
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-zinc-900/90 backdrop-blur-xl border-t border-zinc-800 px-6 pb-[env(safe-area-inset-bottom)] pt-4 flex flex-col gap-4">
        <div className="flex items-center gap-4">
          <span className="text-[10px] text-zinc-500 uppercase font-black tracking-widest">Scroll Speed</span>
          <input 
